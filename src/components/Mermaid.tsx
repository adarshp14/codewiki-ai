    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75 p-4">
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-[var(--border-color)]">
          <div className="font-medium text-[var(--foreground)] font-serif">{messages.diagram?.title || 'Diagram View'}</div>
...
{messages.diagram?.renderError || 'Diagram Rendering Error'}
          </div>
        </div>
        <div ref={mermaidRef} className="text-xs overflow-auto"></div>
        <div className="mt-3 text-xs text-[var(--muted)] font-serif">
          {messages.diagram?.syntaxError || 'There is a syntax error in the diagram and it cannot be rendered.'}
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={`flex justify-center items-center p-4 ${className}`}>
        <div className="flex items-center space-x-2">
          ...
          <span className="text-[var(--muted)] text-xs ml-2 font-serif">{messages.diagram?.loading || 'Rendering diagram...'}</span>
        </div>
      </div>
    );
  }
